...
class ProgressShopViewHodel(v: View) : RecyclerView.ViewHolder(v) {
    ...
    val category = itemView.findViewById<TextView>(R.id.receive_shop_category)
    ...
    fun bindWithView(item: ProgressAllDate, onClickListener: View.OnClickListener) {
        ...
        type_text.setTextColor(Color.parseColor("#0f0f0f"))
        type_text.setText("소품대여")
        type_background.setBackgroundColor(Color.parseColor("#ffffff"))
        date_term.text = item.date_term
        start_time.text = item.start_date
