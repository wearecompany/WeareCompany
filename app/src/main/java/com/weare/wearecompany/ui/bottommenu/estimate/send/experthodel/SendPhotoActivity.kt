...
import com.weare.wearecompany.databinding.ActivitySendPhotoBinding
...
class SendPhotoActivity:BaseActivity<ActivitySendPhotoBinding>(
    ...
    private fun setup() {
        ...
        mViewDataBinding.sendPhotoToolbarRefundMenu.setOnClickListener(this)
        mViewDataBinding.reservationExpertChat.setOnClickListener(this)
        mViewDataBinding.sendPhotoExpertInfoLayout.setOnClickListener(this)
        ...
        sendManager.instance.expertPage(reserve_idx, completion = { responseStatus, data ->
            when (responseStatus) {
                ESTIMATE.OKAY -> {
                    ...
                    mViewDataBinding.sendPhotoExpertName.text = data[0].expert_name
                    mViewDataBinding.sendPhotoExpertPlace.text = data[0].expert_place
                    mViewDataBinding.sendPhotoExpertPrice.text = data[0].expert_price
                    ...
                    mViewDataBinding.sendPhotoExpertCategoryRecyclerview.layoutManager =
                        LinearLayoutManager(
                            this,
                            LinearLayoutManager.HORIZONTAL, false
                        )
                    mViewDataBinding.sendPhotoExpertCategoryRecyclerview.adapter = tagAdapter
                    ...
                    Glide.with(MyApplication.instance)
                        ...
                        .into(mViewDataBinding.sendPhotoExpertImage)
                    ...
                    if (dtlist.size != 0) {
                        mViewDataBinding.sendPhotoExpertRecyclerView.visibility = View.VISIBLE
                        dateadapter = ReservationStudioDialogRecyclerViewAdapter(dtlist)
                        mViewDataBinding.sendPhotoExpertRecyclerView.layoutManager =
                            GridLayoutManager(this, 3)
                        mViewDataBinding.sendPhotoExpertRecyclerView.adapter = dateadapter
                    }

                    mViewDataBinding.sendPhotoExpertTime.text = data[0].reserve_time.toString()
                    mViewDataBinding.sendPhotoExpertHeadcount.text =
                        data[0].reserve_headcount.toString()
                    mViewDataBinding.sendPhotoExpertContents.text = data[0].reserve_contents
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.send_photo_expert_info_layout -> {
