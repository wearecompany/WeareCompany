...
class ChatActivity : BaseActivity<ActivityChatBinding>(
    ...
    private var chat_list_postion = -1
    ...
    private fun setup() {
        when (Entrytype) {
            ...
            1 -> {  //리스트 진입
                ...
                chat_end = intent.getIntExtra("chat_end", 0)
                expert_type = intent.getIntExtra("expert_type", 0)
                status = intent.getIntExtra("status", 0)
                refund_status = intent.getIntExtra("refund_status", 0)
                report_status = intent.getIntExtra("report_status", 0)
                expert_user_idx = intent.getStringExtra("expert_user_idx").toString()

                if (refund_status == 2 || chat_end == 1) {
                    mViewDataBinding.chatEditText.setHint("채팅창이 비활성화 됐습니다.")
                    mViewDataBinding.chatEditText.isClickable = false
                    mViewDataBinding.chatEditText.isFocusable = false
                    mViewDataBinding.chatImageUpload.isClickable = false
                }
            ...
            2 -> {  //상세페이지 진입
                ...
                expert_type = intent.getIntExtra("chat_end", 0)
                expert_user_idx = intent.getStringExtra("expert_user_idx").toString()
                individualsetup()
            }
        }
    ...
    fun detailcall() {
        ...
        when (type) {
            0 -> {
                ...
                `object`.addProperty("request_idx", "")
                `object`.addProperty("request_log_idx", "")
                `object`.addProperty("app_type", 0)


                chattingManager.instance.detail(
                    reserve_idx,
                    "",
                    "",
                    0,
                    completion = { responseStatus, arraylist ->
                        when (responseStatus) {
                            RESPONSE_STATUS.OKAY -> {
                                ...
                                chat_idx = detailArry[0].chat_idx
                                refund_status = detailArry[0].refund_status
                                status = detailArry[0].status
                                report_status = detailArry[0].report_status
                                expert_type = detailArry[0].expert_type
                                expert_user_idx = detailArry[0].expert_user_idx
                                expert_user_nickname = detailArry[0].opponent_nickname
                                mViewDataBinding.expertUserNickname.text =
                                    detailArry[0].opponent_nickname
                                if (arraylist[0].chat_end == 1 || arraylist[0].refund_status == 2) {
                                    mViewDataBinding.chatEditText.setHint("채팅창이 비활성화 됐습니다.")
                                    mViewDataBinding.chatEditText.isClickable = false
                                    mViewDataBinding.chatEditText.isFocusable = false
                                    mViewDataBinding.chatImageUpload.isClickable = false
                                ...
                                if (dataAdapter.itemCount != 0) {
                                    mViewDataBinding.chatRecyclerView.postDelayed(Runnable { // Select the last row so it will scroll into view...
                                        mViewDataBinding.chatRecyclerView.smoothScrollToPosition(
                    ...
                    completion = { responseStatus, arraylist ->
                        when (responseStatus) {
                            RESPONSE_STATUS.OKAY -> {
                                ...
                                mViewDataBinding.expertUserNickname.text =
                                    detailArry[0].opponent_nickname
                                if (arraylist[0].chat_end == 1) {
                                    mViewDataBinding.chatEditText.setText("채팅창이 비활성화 되었습니다.")
                                    mViewDataBinding.chatEditText.isClickable = false
                                    mViewDataBinding.chatEditText.isFocusable = false
                                    mViewDataBinding.chatImageUpload.isClickable = false
                                }
    ...
    fun individualsetup() {
        ...
        chattingManager.instance.individual(
            user_idx.toInt(),
            expert_type.toString(),
            expert_idx,
            expert_user_idx,
            completion = { responseStatus, arrayList ->
                when (responseStatus) {
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.relativeLayout3 -> {
                when (Entrytype) {
                    0 -> {
                        when (detailArry[0].type) {
                            0 -> {
                                if (refund_status != 0) {
                                    when (expert_type) {
                                        0 -> {
                                            newIntent =
                                                Intent(this, RefundStudioActivity::class.java)
                                        }
                                        1 -> {
                                            newIntent =
                                                Intent(this, RefundPhotoActivity::class.java)
                                        }
                                        2 -> {
                                            newIntent =
                                                Intent(this, ReceiveModelActivity::class.java)
                                        }
                                        3 -> {
                                            newIntent =
                                                Intent(this, RefundTripActivity::class.java)
                                        }
                                    }
                                    newIntent.putExtra("reserve_idx", reserve_idx)
                                    newIntent.putExtra("type", 0)
                                    newIntent.putExtra("chatbool", 1)
                                    startActivityForResult(newIntent, 3000)
                                } else {
                                    if (status == 0) {
                                        when (expert_type) {
                                            0 -> {
                                                newIntent =
                                                    Intent(this, SendStudioActivity::class.java)
                                            }
                                            1 -> {
                                                newIntent =
                                                    Intent(this, SendPhotoActivity::class.java)
                                            }
                                            2 -> {
                                                newIntent =
                                                    Intent(this, SendModelActivity::class.java)
                                            }
                                            3 -> {
                                                newIntent =
                                                    Intent(this, SendTripActivity::class.java)
                                            }
                                        }
                                        newIntent.putExtra("reserve_idx", reserve_idx)
                                        newIntent.putExtra("type", status)
                                        newIntent.putExtra("chatcheck", 1)
                                        startActivityForResult(newIntent, 3000)
                                    } else if(status == 1) {
                                        when (expert_type) {
                                            0 -> {
                                                newIntent =
                                                    Intent(this, ReceiveStudioActivity::class.java)
                                            }
                                            1 -> {
                                                newIntent =
                                                    Intent(this, ReceivePhotoActivity::class.java)
                                            }
                                            2 -> {
                                                newIntent =
                                                    Intent(this, ReceiveModelActivity::class.java)
                                            ...
                                            3 -> {
                                                newIntent =
                                                    Intent(this, ReceiveTripActivity::class.java)
                                            }
                                        }
                                        newIntent.putExtra("reserve_idx", reserve_idx)
                                        newIntent.putExtra("type", status)
                                        ...
                                        when (expert_type) {
                                            0 -> {
                                                newIntent =
                                                    Intent(this, ProgressStudioActivity::class.java)
                                            }
                                            1 -> {
                                                newIntent =
                                                    Intent(this, ProgressPhotoActivity::class.java)
                                            }
                                            2 -> {
                                                newIntent =
                                                    Intent(this, ProgressModelActivity::class.java)
                                            }
                                            3 -> {
                                                newIntent =
                                                    Intent(this, ProgressTripActivity::class.java)
                                            }
                                        }
                                        newIntent.putExtra("reserve_idx", reserve_idx)
                                        newIntent.putExtra("type", 0)
                            ...
                            1 -> {
                                if (refund_status != 0) {
                                    newIntent = Intent(this, RefundManyActivity::class.java)
                                    ...
                                    if (status == 0 || status == 1) {
                                        newIntent = Intent(this, ReceiveRequestActivity::class.java)
                                        newIntent.putExtra("request_idx", request_idx)
                                        newIntent.putExtra("log_idx", request_log_idx)
                                        newIntent.putExtra("chatbool", 1)
                                        newIntent.putExtra("type", status)
                                        ...
                                        newIntent = Intent(this, ProgressManyActivity::class.java)
                                        newIntent.putExtra("request_idx", request_idx)
                                        newIntent.putExtra("log_idx", request_log_idx)
                                        newIntent.putExtra("chatbool", 1)
                                        newIntent.putExtra("type", status)
                            ...
                            0 -> {
                                if (refund_status != 0) {
                                    when (expert_type) {
                                        ...
                                        2 -> {
                                            newIntent =
                                                Intent(this, RefundModelActivity::class.java)
                                        }
                                        3 -> {
                                            newIntent =
                                                Intent(this, RefundTripActivity::class.java)
            ...
            R.id.chat_menu -> {
                val chatmenudialog: ChatDialogFragment = ChatDialogFragment() {
                    when (it) {
                        0 -> {
                            chattingManager.instance.out(chat_idx, completion = { responseStatus ->
                                when (responseStatus) {
                                    RESPONSE_STATUS.OKAY -> {
                                        val intent = Intent()
                        ...
                        1 -> {
                            if (report_status != 1) {
                                newIntent = Intent(this, ReportActivity::class.java)
                                newIntent.putExtra("chat_idx", chat_idx)
                                newIntent.putExtra("claim_idx", user_idx)
                                newIntent.putExtra("defandant_idx", expert_user_idx)
                                newIntent.putExtra("expert_user_nickname", expert_user_nickname)
                                newIntent.putExtra("type", type)
                                startActivityForResult(newIntent, 4000)
                            } else {
                                Toast.makeText(this, "이미 신고하였습니다.", Toast.LENGTH_SHORT).show()
                            }
