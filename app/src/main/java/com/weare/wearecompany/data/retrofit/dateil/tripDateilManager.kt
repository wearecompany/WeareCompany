...
class tripDateilManager {

    companion object {
        val instance = tripDateilManager()
    }
    ...
    //메인 데이터 호출
    fun date(user_idx: String?, expert_idx: String?, completion: (RESPONSE_STATUS, ArrayList<dateilTrip>) -> Unit) {

        val `object` = JsonObject()
        `object`.addProperty("user_idx", user_idx)
        `object`.addProperty("expert_idx", expert_idx)

        val call = iRetrofit?.dateitrip(jsonObject = `object`).let {
            it
        } ?: return

        call.enqueue(object : retrofit2.Callback<dateilTrip> {

            // 응답 실패시
            override fun onFailure(call: Call<dateilTrip>, t: Throwable) {
                Log.d(Constants.TAG, "RetrofitManager - onFailure() called / t: $t")
            }

            // 응답 실패시
            override fun onResponse(call: Call<dateilTrip>, response: Response<dateilTrip>) {

                response.body()?.let {

                    var dateilArray = ArrayList<dateilTrip>()

                    val dateilItem = dateilTrip(
